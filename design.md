...
- **技術**:
    - **再生ボタンが押された時**:
        ...
        - **`AudioWorklet`**: ここで`libsamplerate-js`（Wasm）などを使って実際の音声データのリサンプリングと再量子化を行う。これにより、メインスレッドのブロックを防ぎ、スムーズな再生を実現。
...
- **フロントエンド**:
    ...
    - **`libsamplerate-js` (Wasm)**: 高品質なリサンプリングを行うためのライブラリ（`AudioWorklet`内で利用）。
- **開発ツール**:
    - **npm**: パッケージ管理。
